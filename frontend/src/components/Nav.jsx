...
function Nav() {
    ...
    const handleSearchItems=async () => {
      try {
        console.log(`[DEBUG] Nav.jsx searching for query: "${query}" in city: "${currentCity}"`);
        const result = await axios.get(`${serverUrl}/api/item/search-items`, {
            params: { query, city: currentCity },
            withCredentials: true
        });
        console.log("[DEBUG] Nav.jsx search response:", result.data);
        dispatch(setSearchItems(result.data));
      } catch (error) {
        console.log("[DEBUG] Nav.jsx search error:", error.response?.data || error.message);
      }
    }
